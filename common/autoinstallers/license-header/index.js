...
function addLicenseHeader(targetDir, licenseContent, options = {}) {
  ...
  function processFile(filePath) {
    ...
    try {
      ...
      const shebangMatch = originalContent.match(recognizedShebang);
      const contentAfterShebang = shebangMatch
        ? originalContent.slice(shebangMatch[0].length)
        : originalContent;

      // Check if the license already exists (simple match at the beginning)
      if (
        !force &&
        (licenseHeaders.some(_header =>
          contentAfterShebang.startsWith(_header),
        ) || contentAfterShebang.startsWith(rushPreHeader.trim()))
      ) {
        return;
      }
      ...
      if (shebangMatch) {
        const lineEnding = shebangMatch[0].endsWith("\r\n") ? "\r\n" : "\n";
        const shebangLine = shebangMatch[0].replace(/\r?\n$/, "");
        const styledLicensedText = licensedText.replace(/\n/g, lineEnding);
        newContent =
          shebangLine + lineEnding + styledLicensedText + contentAfterShebang;
      } else {
