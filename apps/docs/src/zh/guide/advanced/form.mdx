...
```tsx pur
const field = useCurrentFieldState()
```
...
```tsx pur
{ render: ({form}) => <div>{form.getValueIn('my.path')}</div> }
```
...
{ render: () => <div><Field name={'my.path'}><MySelect /></Field></div> }

// MySelect.tsx
...
const form = useForm()
form.getValueIn('my.other.path')
...
```
...
该 hook 和上述 `useForm`, 在 `render` 函数返回组件的第一层不生效，封装过的组件内部可用。如果需要在 `render` 根级别使用，可以对内容做一层组件封装。
...
`<TValue = any>(node: FlowNodeEntity) => TFormValues | undefined`

```tsx pur
const value = useWatchFormValueIn(node)
```
...
```tsx pur
const value = useWatchFormState(node)
```
