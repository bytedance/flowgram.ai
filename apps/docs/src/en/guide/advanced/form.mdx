...
[> Array example](https://flowgram.ai/examples/node-form/array.html)

#### FieldArray (Model)

`FieldArray` inherits from `Field`, it's the rendering level model for array type fields. Besides common rendering level APIs, it also includes basic array operations like `FieldArray.map`, `FieldArray.remove`, `FieldArray.append`, etc. API usage can also be found in the above [array example](https://flowgram.ai/examples/node-form/array.html).
...
[> Linkage example](/examples/node-form/dynamic.html)
